...
def eprank(G, M, num_iters, player, pv, w=0):
    """
    ...
    # prior skill variance (prior mean is always 0) is pv
    ...
    # initialize matrices of skill to game to game messages, means and precisions
    Msg = np.zeros((N, 2))
    Psg = np.zeros((N, 2))
    precisions = np.zeros(num_iters)
    means = np.zeros(num_iters)
    for iter in range(num_iters):
        ...
        # Store mean and precision for
        means[iter] = Ms[player]
        precisions[iter] = Ps[player]
    return Ms, Ps, means, precisions
