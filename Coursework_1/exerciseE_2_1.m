...
for t = 1:N
    hyp = struct('mean',[], 'cov', [ 0.3694, 11.7665, 0.1041, 9.8185, -0.0146, -0.3421 ], 'lik', -2.3246);
...
%fill3([test_x(:,1); flip(test_x(:,1),1)], [test_x(:,2); flip(test_x(:,2),1)], f, [7 7 7]/8);
%{
...
%}
subplot(2,1,1);
test_x = [linspace(-a, a, T)', linspace(-a, a, T)'];
size(test_x)
size(f)
